            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="w-full max-w-xl">
                        <form class="bg-white rounded px-8 pt-6 pb-8 mb-4" action="{{ route('quizzes.update', $quiz->id) }}" method="post">
                            @method('PUT')
                            @csrf
                            ...
                            <div class="mb-4">
                                ...
                                <input class="form-control shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('title') is-invalid @enderror" name="title" id="title" type="text" value="{{ old('title') ?? $quiz->title }}">
                                @error('title')
                          ...
                          <div class="mb-6">
                            ...
                            <textarea name="description" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline  @error('description') is-invalid @enderror" id="description" rows="5">{{ old('description') ?? $quiz->description }}</textarea>
                            @error('description')
                            <div class="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4" role="alert"> {{ $message }} </div>
                            @enderror
                          </div>
                          ...
                          <div class="mb-6">
                            ...
                            <div>
                                <div class="inline">
                                    <input class="isFinished" type="radio" name="isFinished" id="isFinished1" {{ old('isFinished') ? (old('isFinished') == "yes" ? "checked" : "") : ($quiz->finished_at != "" ? "checked" : "") }} value="yes">
                                    <label class="inline" for="isFinished1">{{ __('general.available') }}</label>
                                </div>
                                <div class="inline">
                                    <input class="isFinished" type="radio" name="isFinished" {{ old('isFinished') ? (old('isFinished') == "no" ? "checked" : "") : ($quiz->finished_at == "" ? "checked" : "") }}  id="isFinished2" value="no">
                            ...
                            <div class="{{ old('isFinished') == "yes" ? "block" : "hidden" }}" id="finish_date">
                                ...
                                <input type="datetime-local" class="@error('finished_at') is-invalid @enderror" name="finished_at" id="finished_at" value="{{ old('finished_at') ? old('finished_at') : ($quiz->finished_at != null ? date('Y-m-d\TH:i', strtotime($quiz->finished_at)) : "") }}">
                                @error('finished_at')
                                <div class="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4" role="alert"> {{ $message }} </div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <select class="form-control shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="status" id="status">
                                    <option @if ($quiz->for_lab != "yes")
                                                @if($quiz->questions_count < 3)
                                                     disabled 
                                                @endif 
                                            @elseif ($quiz->for_lab == "yes" )
                                                @if($quiz->questions_count < 3)
                                                     disabled 
                                                @endif 
                                            @endif value="published" @if(old('status')){{ old('status') == "published" ? "selected" : "" }}@else{{ $quiz->status == "published" ? "selected" : "" }}@endif>{{ __('quiz.published') }}</option>
